 ...
 *
 * Added by jkhoury@bbn.com: May 2016
 * @see ReadableJsonEncoder
 *
 * After starting to work on it, I realized how silly the {@link JsonEncoder}
 *   implementation is with all its fields private -- so it is technically
 *   impossible to override the behavior of readFixed and readBytes without
 *   create a complete copy of {@link JsonEncoder}
 *
 */
 ...
 * */
public class ReadableJsonDecoder extends ParsingDecoder
    ...
    private ReadableJsonDecoder(Symbol root, InputStream in) throws IOException {
        super(root);
        configure(in);
    }

    private ReadableJsonDecoder(Symbol root, String in) throws IOException {
        super(root);
        configure(in);
    }

    // jkhoury@bbn.com: made this public
    public ReadableJsonDecoder(Schema schema, InputStream in) throws IOException {
        this(getSymbol(schema), in);
    }

    // jkhoury@bbn.com: made this public
    public ReadableJsonDecoder(Schema schema, String in) throws IOException {
        this(getSymbol(schema), in);
    }
     ...
     */
    public ReadableJsonDecoder configure(InputStream in) throws IOException {
        if (null == in) {
     ...
     * @return this JsonDecoder
     */
    public ReadableJsonDecoder configure(String in) throws IOException {
        if (null == in) {
    ...
    private byte[] readByteArray() throws IOException {
        /**
         * jkhoury@bbn.com: updated this to parse UUID/hex string instead
         * @see {@link ReadableJsonEncoder}
         */
        /*byte[] result = in.getText().getBytes(CHARSET);
        return result;*/
        String textString = this.in.getText();
        if (textString.length() == UUID_STRING_LENGTH
                && textString.charAt(UUID_HYPHEN_POSITION1) == '-'
                && textString.charAt(UUID_HYPHEN_POSITION2) == '-'
                && textString.charAt(UUID_HYPHEN_POSITION3) == '-'
                && textString.charAt(UUID_HYPHEN_POSITION4) == '-') {
            return Utils.fromUuidString(textString);
        }
        return Utils.fromHexString(textString);
    }
