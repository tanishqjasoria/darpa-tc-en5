...
public class UUIDCheck extends CDMChecker {
	...
	seenUuids = new HashMap<Class<?>, HashQueue<BigIntWithEpoch>>();
        ...
        useBeforeDefine = 0;
        logger.info("Queues initialized with "+hashQueueCapacity);
    }
    ...
    public void processRecord(String key, TCCDMDatum datum) throws Exception {
        ...
        if(record instanceof Principal) {
            ...
            if (subject.getParentSubject() != null) {
                ...
                if (!psUuid.getUuid().equals(zeroBI) && !subjectUuids.contains(psUuid) && !uuid.equals(subject.getParentSubject())) {
                    useBeforeDefine++;
            ...
            if (subject.getLocalPrincipal() != null) {
                ...
                if (!pUuid.getUuid().equals(zeroBI) && !principalUuids.contains(pUuid) && !hostUuids.contains(pUuid)) {
                    useBeforeDefine++;
        ...
        if (uuid != null || sUuid != null) {
            ...
            if (typeUuids.contains(sUuid)) {
                ...
                typeUuids.add(sUuid);
                if (sUuid.getEpoch() > 0) {
                    // Also add a zeroEpoch version for wildcard matching (references may not include the epoch)
                    BigIntWithEpoch zeroEpoch = new BigIntWithEpoch(sUuid.getUuid());
                    // Do we already have one?
                    if (!typeUuids.contains(zeroEpoch)) {
                        typeUuids.add(zeroEpoch);
                    }                    
                }
            }
        }
    ...
    public void close() {
        ...
        // Capacities
        for (Class<?> c : seenUuids.keySet()) {
            HashQueue<BigIntWithEpoch> hq = seenUuids.get(c);
            logger.info("Class: "+c+" HashQueue size: "+hq.size());
        }
    }    
